...
/// The extractor's own notices: every `# ...` line it writes to stderr, which is
/// how many files it is walking, which ones it skipped, and (#66) how many
/// entities it could not attribute to a file. `--yes` pipes stderr so a FAILED
/// build can explain itself at the next session start via `.last-error`, and
