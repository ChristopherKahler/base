...
fn handoff_slug_defaults_to_doc_basename() {
    ...
    let slug = crud::handoff::create(None, tmp.path(), tmp.path(), &ns(), "base-v2", doc, None)
        .unwrap()
        .slug;
...
fn explicit_slug_override_wins_for_both() {
    ...
    let hslug = crud::handoff::create(
        None,
        tmp.path(), tmp.path(), &ns(), "base-v2", "/d/whatever-timestamped.md", Some("resume-base-v2"),
    )
    .unwrap()
    .slug;
...
fn recreate_same_slug_is_idempotent() {
    ...
    crud::handoff::create_fork(tmp.path(), &ns(), "p", "/d/FEATURE-A.md", None).unwrap();
    crud::handoff::create(None, tmp.path(), tmp.path(), &ns(), "p", "/d/RESUME.md", None).unwrap();
    crud::handoff::create(None, tmp.path(), tmp.path(), &ns(), "p", "/d/RESUME.md", None).unwrap();
...
fn fork_and_handoff_are_independent() {
    ...
    crud::handoff::create(None, tmp.path(), tmp.path(), &ns(), "base-v2", "/d/HANDOFF-1.md", None).unwrap();
    crud::handoff::create_fork(tmp.path(), &ns(), "base-v2", "/d/FEATURE-A.md", None).unwrap();
    ...
    crud::handoff::create(None, tmp.path(), tmp.path(), &ns(), "base-v2", "/d/HANDOFF-2.md", None).unwrap();
    let store = store_of(tmp.path());
    let open_handoffs = count(
        &store,
        &format!(
...
fn list_surfaces_run() {
    let tmp = workspace();
    crud::handoff::create_fork(tmp.path(), &ns(), "p", "/d/FEATURE-A.md", None).unwrap();
    crud::handoff::create(None, tmp.path(), tmp.path(), &ns(), "p", "/d/HANDOFF-1.md", None).unwrap();
