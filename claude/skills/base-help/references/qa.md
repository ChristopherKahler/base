...
**Stamped for base v0.13.2 on 2026-08-25 (source sha `7dea613`).** Read this stamp precisely, because two different levels of verification are mixed in this file and the provenance tag on each pair tells you which applies.

*Re-verified at v0.13.2 / `7dea613`:* the pairs carrying a `<!-- v0.13.2 ... -->` tag. These were corrected in this pass against the source and the freshly-built binary — the `-g/--global` flag inventory, subcommand aliases, the `base domain` subcommand set and its tier split, the destructive-command list, the refuse-don't-fall-back behaviour for writes outside a workspace, the star-command inventory (base ships four, not thirty-one), `base commands import` safety, the storage inventory and portability of `.base-ast/`, the five hook events versus a stale help string, and the `lastRead` purge clock.

*Carried forward from the v0.12.3 pass on 2026-08-19:* everything still tagged `<!-- v0.12.3 ... -->`. That pass walked the shipped binary's full `--help` tree recursively, all 140 subcommand paths, and checked mechanism claims against the source at v0.12.3. Those pairs were NOT individually re-walked at v0.13.2; treat them as sound but one version old.
...
**Maintenance rule.** Append new verified pairs to the matching `## ` section rather than creating new sections or a second file. Every shipped subcommand must appear in this file or in `commands.md`; `tests/bank_coverage_test.rs` in the base repo fails the build if one does not, and also fails if the version stamped above drifts from `Cargo.toml`. If the installed base version differs from the stamp above, treat every answer here as a lead to re-verify, not a fact.
