...
### Q: Who makes base, and where do I get help?
**A:** base is built by Chris Kahler. The documentation is at `https://docs.basemode.ai`, and bugs and questions go to the issue tracker at `https://github.com/ChristopherKahler/base/issues`. The docs link and the credit are printed at the bottom of `base --help`.
...
**A:** `base handoff list` and `base fork list`, both list across the global and workspace tiers and show which tier each entry lives in. To open one handoff, run `base handoff show` with whatever you have: a letter from the session start list (A to J), its slug, its project name, or a few words ("skyrim companion"). It prints the doc path on its first line and says what matched; when several handoffs match it lists them all and picks none (exit 2), and when none match it exits 1. It reads and never writes. There is no `fork show`: `cat` the doc path `base fork list` gives you.
<!-- v0.16.0 | verified: source -->
...
**A:** You mostly don't have to do anything: an open handoff resurfaces automatically in the injected context at the start of your next session, and it keeps resurfacing every session until you pick it up, snooze it, or archive it. If you have several open handoffs or forks across projects, just tell Claude "resume the X work": the session start tells it to run `base handoff show <what you said>` and read the doc that prints, and forks are read from `base fork list`.
<!-- v0.16.0 | verified: source -->
...
**A:** `base domain create --name <NAME> --keyword <K> --path <P>` creates it in the tier you are standing in: the workspace `.base/domains.toml` when the cwd is inside a registered workspace, the global `~/.base-gbl/domains.toml` outside one or with `--global`, and it says which (`Domain 'X' created (workspace tier)`). Since v0.14.1 (#61) the four write commands, `create`, `remove`, `add-trigger` and `remove-trigger`, resolve the file the same way, each reports what it changed, and one that changed nothing names the tier it searched and the one it did not, then exits non-zero. `base domain` has seven subcommands: `add-trigger`, `list`, `get`, `sync`, `create`, `remove`, and `remove-trigger`. To add a trigger to an existing domain use `base domain add-trigger --domain <name> [--keyword <kw>] [--path <path>]`; a path that covers two or more registered projects is refused (see the broadcast pairs at the end of this section). Hand-editing the right `domains.toml` also works (the trigger keys are `prompt_keywords`, `file_keywords`, and `paths`); either way run `base domain sync` to push triggers and any bootstrap rule content into the graph (this also runs automatically via a timestamp-gated marker on session start). Confirm the subcommand set with `base domain --help`.
<!-- v0.14.2 | verified: source+binary -->
...
**A:** `base domain add-trigger --domain <name> [--keyword <kw>] [--path <path>]` adds one, and `base domain remove-trigger --domain <name> [--keyword <kw>] [--path <path>]` takes it back off. Both edit `domains.toml`, which holds triggers only; the rule *content* lives in the graph and is managed separately with `base rule add`. Since v0.14.1 (#61) the two act on the same file, the tier you are standing in (the workspace file inside a registered workspace, the global file outside one or with `--global`), so a trigger you added is removable from the CLI again; through v0.14.0 `add-trigger` wrote the workspace file while `remove-trigger` only read the global one. A remove that finds nothing to remove says which tier it searched and which it did not, and exits non-zero instead of reporting success. Verify with `base domain get <name>`, or better, `base context "<some text>"` to see whether the new trigger actually fires.
<!-- v0.14.2 | verified: source+binary -->

### Q: How do I delete a domain I no longer use?
**A:** `base domain remove <name>`, case-insensitive on the name. Since v0.14.1 (#61) it removes the domain's entry from the `domains.toml` of the tier you are standing in (the workspace file inside a registered workspace, the global file outside one or with `--global`), reports which (`Domain 'X' removed (workspace tier)`), and when the domain is not in that file it names the tier it searched and the one it did not and exits non-zero, so a domain that `base domain list` shows is always removable from where it lives. Two things to know. First, this is a triggers-level delete: rules previously added to that domain still exist as graph entities, they simply have nothing left to fire them. Second, if you want a domain quiet but recoverable, removing its triggers, or setting `auto_inject = false`, is the softer move than removing the domain.
<!-- v0.14.2 | verified: source+binary -->

### Q: How do I keep a domain out of automatic injection but still reachable when I ask for it?
**A:** Set `auto_inject = false` on the domain in `domains.toml`, in whichever tier it lives. Since v0.14.1 that keeps the domain out of every automatic surface whatever its mode or triggers: the user-prompt-submit hook, the pre-tool-use hook and the session-start cheat-sheet all drop it before matching. Anything you ask for explicitly still sees it: `base context "<some text>"`, `base recall --keyword <kw>`, `base domain get <name>`, `base rule list --domain <name>` and star commands. The typical use is a domain whose rules carry terms that must never reach an unrelated prompt (a negotiation, a client's numbers). One ordering rule for machines that share the file: install v0.14.1 or later on every one of them first, then set the flag. v0.14.0 ignored the key, and its `add-trigger` rewrote `domains.toml` without the keys it did not know, so a 0.14.0 machine touching the file would have dropped the flag.
<!-- v0.14.2 | verified: source+binary -->

### Q: How does a path trigger decide whether it matches, since v0.14.1?
**A:** Against the files this session has touched through the tool hooks (the `pre-tool-use` rows in `hook-events.jsonl` carrying this session's id) plus the session's cwd, and nothing else. v0.14.0 matched against every path the store had ever seen active, every registered project, task and handoff, on every prompt, which is why a trigger named `Documents` fired on prompt 3 of every session. The comparison is a prefix match on path components: `Documents` covers `Documents/x` and never `Documents-old/x` or `MyDocuments/x`. It is case-blind whenever either side is a Windows path (one with a drive letter), so `Tools` and `tools` are one directory there, and `/mnt/c/...` and `C:/...` are read as the same drive. A relative trigger resolves against the tier its file came from, the home for the global `domains.toml` and the workspace root for the workspace one; `~` resolves against the home; an absolute path is taken as written; a glob cannot be rooted and never fires (see the next pair). `prompt_keywords` match whole words, so `base` no longer fires on `database`. With devmode on, the reason under `LOADED DOMAINS` names the file that satisfied the trigger.
<!-- v0.14.2 | verified: source+binary -->

### Q: Why did my domain stop loading after v0.14.1?
**A:** Almost always because one of its path triggers is a broadcast. A path trigger that is a prefix of the paths of two or more registered projects (`Documents` on a home with ten projects under Documents) cannot say which project you are in, so since v0.14.1 it is inert: it fires in neither hook, and a domain that relied on it stops loading with no other sign. The same holds for an unrooted trigger, a glob or a relative path in a file with no tier root. Three places tell you. `base doctor` prints a `path triggers` section that names the domain, the trigger and the registered projects it covers, per tier, and counts every inert trigger against the verdict. With devmode on, the prompt hook prints one `inert:` line per faulty trigger on every prompt. And `base domain add-trigger --domain <name> --path <path>` refuses to add such a trigger in the first place. The fix is one line in `domains.toml`: narrow the trigger to the one project's path, or set `auto_inject = false` if the domain should not fire on paths at all. A project registered in both tiers counts once.
<!-- v0.14.2 | verified: source+binary -->

### Q: Why was my add-trigger refused, and what happened to the file?
**A:** Since v0.14.1 `base domain add-trigger --domain <name> --path <path>` checks the path against the registered projects before it writes. If the path covers two or more of them, or cannot be rooted, the command prints the sentence `base doctor` prints for an inert trigger (``path trigger `Documents` on `<name>` covers 10 registered projects (...); narrow it or set auto_inject = false``), exits non-zero, and writes nothing: `domains.toml` is byte for byte what it was. `base project add --name <name> --path <path>` under such a path still registers the project but creates no same-named domain, and says why (`no domain was created`), because the domain it would have created could never fire. Keyword triggers are never refused this way; only a path can be a broadcast.
<!-- v0.14.2 | verified: source+binary -->

### Q: Does exclude veto an always-on domain?
**A:** Yes, since v0.14.1. `exclude` patterns are checked first for every domain, and a match vetoes the domain whatever its mode, an `always` domain included. Before v0.14.1 an always-on domain matched before the exclude check ran, so `exclude` on it was dead configuration. An `exclude` entry matches as a case-insensitive substring of the prompt, on purpose looser than a keyword's whole-word match: a veto that fires too often errs toward silence.
<!-- v0.14.2 | verified: source+binary -->
...
**A:** A pipeline including: a graph-health warning if unhealthy, tier auto-compaction if bloated, clearing this session's own dedup state, domain sync, ingesting `paul.toml` projects, active/deferred state reconciliation, an operator profile block, an update banner, all signals (memory, active-awareness, pulse, flow-resurface, handoff, reminder, fork), the flow protocol block, extension status, a context-triggers cheat sheet, and relay inbox/task delivery. All of it is collected and printed once. Line 1 is a header with every count and the path of `.base/last-session-start.md`, which holds the untrimmed output; then the instruction block, DUE NOW, up to 10 lettered handoffs (newest first, one per project), the 3 newest forks, and PROJECTS, TASKS and MILESTONES. Those three count every working item and list only what is on a project touched in the last `[session_start] recent_project_days` days (7): that is what "in progress" means there, since no task status says it. Everything else follows, and if the output is over `[budget] session_start_chars` (9000) the bottom blocks shrink to one line each, naming the command or the file that holds the rest.
<!-- v0.16.0 | verified: source -->
...
**A:** A layered system: (1) domain matching against prompt keywords, file keywords, and active file paths, deterministic substring/path matching, nothing fuzzy or semantic, and never context-size-aware; (2) suppression, each signal's output is hashed and unchanged output is skipped, so silence is the default steady state; (3) a size budget, `[budget] session_start_chars` (default 9000, counted in the UTF-16 units Claude Code counts), over everything session start prints: nothing is exempt, blocks at the bottom shrink to one line naming where the rest is, and the old `signal.max_chars` is read by nothing; the memory block also has its own budget inside that one, `[budget] memory_chars` (default 4000), filled with whole notes, corrections first and newest first, then one line counting the rest and naming `base learn --list`; handoffs, reminders and forks are still exempt from suppression and surface every session until acted on; (4) the bracket system layering rule-injection intensity on top of session depletion. Layers 1 to 3 are purely trigger- and content-based. Layer 4 is the exception: in percent mode (what `base install` writes by default) the bracket reads real context depletion from the live transcript's usage block, so injection volume does adapt to how full your context window is. See the bracket pair below for the tiers and thresholds.
<!-- v0.16.0 | verified: source -->
...
**A:** Run `base install`. It builds the binary, symlinks it globally, creates `~/.base-gbl`, wires hooks into `settings.json`, and writes the manifest. Options: `--carl <path>` to migrate decisions from a `carl.json` file, `--skip-hooks` to skip wiring hooks into `settings.json`, and `--full` to register the bundled components (PAUL, SEED, SKILLSMITH) in the manifest, not just base itself.
<!-- v0.12.3 | verified: cli-help -->

### Q: What does `base activate` do, and do I need it?
**A:** Nothing, and no. It used to take a key that removed attribution branding from base's output. base no longer carries that branding, so there is nothing left for the key to remove and the feature is retired. The command still parses and still exits 0 -- a provisioning script that runs `base activate $KEY` should not start failing -- and prints one line saying activation is no longer required. A manifest that already records a `token` or `source` from the old feature still loads; nothing writes those fields again.
<!-- v0.13.19 | verified: cli-help|source -->

### Q: I just installed base. What do I actually do next?
**A:** Open a new session and work normally -- there is nothing to configure first, and base learns from what you decide and correct. `base install` and `base scaffold` both print the same short message saying so. When you want more than that, `base help getting-started` covers setting a folder up as a workspace, domain triggers and rules, running more than one session on the relay, star commands, and migrating from CARL. The full documentation is at `https://docs.basemode.ai`.
<!-- v0.13.19 | verified: cli-help -->
...
**A:** Run `base update --check`. It re-validates against the remote source and reports whether a newer version exists, without installing anything. For the `base` component specifically, this compares against GitHub releases; the bundled components are checked against npm.
<!-- v0.12.3 | verified: cli-help|source -->
...
**A:** Public GitHub releases of the base repository, and nothing else. There is no license check, no account, and no identifier leaves the machine -- the only request is for the release list. `base` itself is checked against GitHub releases (`fetch_github_version`); the bundled components (PAUL, SEED, SKILLSMITH) are checked against their npm packages. (Before v0.13.0 this went through a license-gated channel that both sent personal data on a routine version check and pointed somewhere the release pipeline never published to, so `base update` could not deliver the releases actually being cut. That is gone.)
<!-- v0.13.19 | verified: source -->
...
**A:** **Fixed in v0.14.1 (#61).** The bug, live through v0.14.0: the four `base domain` write commands did not agree on which tier they operated on. `base domain add-trigger` resolved the workspace via `find_workspace_base(cwd)` and wrote `{workspace}/.base/domains.toml`, creating both the file and the domain entry if they were missing, while `base domain create`, `base domain remove` and `base domain remove-trigger` ignored cwd and only ever touched the global `~/.base-gbl/domains.toml`. So a domain or trigger created by `add-trigger` inside a workspace could not be removed by any CLI command: `remove` and `remove-trigger` reported "Domain '<name>' not found" while `base domain list` kept showing it, because `load_domains` merges both tiers for reads. Since v0.14.1 one resolver decides the file for all four writes (the workspace file inside a registered workspace, the global file outside one or with `--global`), every write reports what it changed and in which tier, and a command that changed nothing names the tier it searched and the one it did not and exits non-zero. If you still see a not-found, you are standing in the other tier: add `--global`, or run the command from inside the workspace the domain lives in.
<!-- v0.14.2 | verified: source+binary -->
