...
def extract_markdown(path: Path) -> dict:
    ...
    When frontmatter contains `ontology: true`, performs rich extraction per the
    Markdown Ontology Protocol (MOP): type, domain, tags, related, and all other
    frontmatter keys become graph properties and edges.

    Always produces structural nodes for headings and fenced code blocks.
    """
    try:
    ...
    def add_node(nid: str, label: str, line: int, file_type: str = "document", **extra) -> None:
        if nid not in seen_ids:
            seen_ids.add(nid)
    ...
    if is_ontology_doc:
        onto_type = _kebab_to_pascal(frontmatter.get("type", "Document"))
        add_node(file_nid, path.name, 1, file_type="ontology_document",
                 ontology_type=onto_type, ontology_meta=frontmatter)
    else:
        add_node(file_nid, path.name, 1)
    ...
    if is_ontology_doc:
        ...
        if supersedes:
            ...
            add_edge(file_nid, sup_nid, "supersedes", 1, confidence="FRONTMATTER")
...
def extract_bash(path: Path) -> dict:
    ...
    def _sanitize_metadata(d: dict) -> dict:
        return {k: str(v)[:256] for k, v in d.items() if v is not None}
    ...
    def add_node(nid: str, label: str, line: int, kind: str = "code") -> None:
        if nid and nid not in seen_ids:
            ...
            nodes.append({"id": nid, "label": label, "file_type": "code",
                          "source_file": str_path, "source_location": f"L{line}",
                          "metadata": _sanitize_metadata({"language": "bash", "kind": kind})})  # noqa: E501
